...
export const useStoreService = () => {
  ...
  const useAddStore = () => {
    return usePost<{ store: Store; token: string }, Store>(STORE.ADD_STORE, {
  ...
  const useAddVendor = () => {
    return usePost<{ store: Store }>(STORE.ADD_VENDOR, {
      onSuccess: (response: any) => {
        // handleSuccess(response.message || "Store added successfully!");
      },
      onError: (error) => {
  ...
  const useGetStores = (enabled: boolean = false, page: number = 1) => {
    ...
    return useGet<{ store: Store; token: string }>(["store"], url, {
      enabled,
      staleTime: 0,
    });
  };
