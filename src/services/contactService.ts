...
export const useContactService = () => {
    ...
    const useAddToNewletter = () => {
        return usePost<{ email: string }>(
            CONTACT_US.ADD_TO_NEWSLETTER,
            {
                onSuccess: (response: any) => {
                    handleSuccess(response.message || "Email added to newsletter successfully!");
                },
                onError: (error) => {
     ...
     const useContactUs = () => {
        return usePost<{ contact: Contact_us}>(
