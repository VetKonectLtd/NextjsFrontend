...
export const useInviteService = () => {
  ...
  const useSendInvite = () => {
    return usePost<any>(INVITATIONS.SEND_INVITE, {
  ...
  const useGetOverallLeaderBoard = (enabled: boolean = false) => {
    return useGet<any>(["leaderboard"], `${Leaderboard.GET_LEADERBOARD}`, {
      enabled,
      staleTime: 0,
    });
  };
  ...
  const useGetUserPoints = (enabled: boolean = false) => {
    return useGet<any>(["userPoints"], `${INVITATIONS.GET_USER_POINTS}`, {
      enabled,
      staleTime: 0,
    });
  };
