...
export const useGeolocation = (): UseGeolocationReturn => {
  ...
  useEffect(() => {
    console.log(
      "🌍 useGeolocation: Hook initialized, checking for geolocation support...",
    );

    // Check if geolocation is supported
    if (!navigator.geolocation) {
      console.error(
        "❌ useGeolocation: Geolocation is not supported by this browser",
      );
    ...
    console.log(
      "✅ useGeolocation: Geolocation is supported, requesting location...",
    );
    setLoading(true);
    setError(null);
    ...
    console.log(
      "⏳ useGeolocation: Location request sent, waiting for response...",
    );
  }, []);
