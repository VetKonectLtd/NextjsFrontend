...
const CaseDetailsPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const caseDataRaw =
          (caseRes as any).case || (caseRes.data && caseRes.data.case);
        if (caseDataRaw) {
  ...
  const DetailRow = ({
    ...
    <div className="flex items-center mb-6">
      <span className="text-[#1A1A1A] font-bold w-[200px] shrink-0">
        {label}:
      </span>
      {isLink ? (
        value && value === "View Image" ? (
          ...
          <span className="text-gray-600 font-normal break-words flex-1">
            {value || "N/A"}
          </span>
        )
      ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-20 gap-y-2">
          ...
          <div>
            ...
            <DetailRow
              ...
              value={caseData.client_phone_number}
            />
            ...
            <DetailRow
              label="Date Occured"
              value={new Date(caseData.date_occurred).toLocaleDateString()}
            />

            <DetailRow
              label="Clinical signs"
              value={
                Array.isArray(caseData.clinical_signs)
                  ? caseData.clinical_signs.join(", ")
                  : caseData.clinical_signs
              }
            />

            <DetailRow label="Heart Rate" value={caseData.heart_rate} />
            <DetailRow
              label="Differential Diag."
          ...
          <div>
            ...
            <DetailRow
              label="Pet Number"
              value={
                caseData.pet_number || caseData.case_id || `#${caseData.id}`
              }
            />

            <DetailRow
              label="Date Presented"
              value={new Date(caseData.date_presented).toLocaleDateString()}
            />
            <DetailRow label="Temperature" value={caseData.temperature} />
            <DetailRow label="Weight" value={caseData.weight} />
            <DetailRow
              label="Tentative Diag."
              value={caseData.tentative_diagnosis}
            />
            <DetailRow
              label="Treatment"
              value={
                Array.isArray(caseData.treatment_regimen)
                  ? caseData.treatment_regimen.join(", ")
                  : caseData.treatment_regimen
              }
            />
            <DetailRow
              label="Image"
              value={
                caseData.picture_url
                  ? "View Image"
                  : typeof caseData.picture === "string"
                    ? "View Image"
                    : "Uploaded"
              }
              isLink
