...
const CasesPage = () => {
  ...
  const fetchCases = async (page: number) => {
    try {
      ...
      const success =
        ...
        (response.data && response.data.cases);
  ...
  return (
    <div className="w-11/12 mt-3 m-auto bg-white min-h-screen pb-20">
      ...
      >
        <span className="text-gray-900 text-sm font-bold ml-2">
          Add New Case
        </span>
        <div className="w-8 h-8 flex items-center justify-center rounded-xl transition">
          {" "}
          {/* Reduced icon container */}
