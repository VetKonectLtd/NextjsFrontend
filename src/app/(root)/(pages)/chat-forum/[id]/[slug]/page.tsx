...
async function getForumPostData(id: string, slug: string) {
	try {
		console.log("Fetching forum post with slug:", slug);
...
export async function generateMetadata({ params }: { params: { id: string, slug: string } }): Promise<Metadata> {
	...
	if (!post) {
		console.log("No post found, returning default metadata");
		return {
			title: "Forum Post Not Found | Vet Konect",
			description: "The requested forum post could not be found or may have been removed.",
			robots: {
				index: false,
				follow: true,
			},
	...
	// Clean content for description (strip HTML and truncate)
	const plainTextContent = post.content?.replace(/<[^>]*>/g, '') || '';
	const description = plainTextContent.substring(0, 160) + (plainTextContent.length > 160 ? '...' : '');

	return {
		title: `${post.title} | Forum Discussion | Vet Konect`,
		description: description || "Join the discussion on this forum post at Vet Konect.",
		openGraph: {
			title: post.title,
...
export default async function ForumDetailPage({ params }: { params: { id: string, slug: string } }) {
	const { id, slug } = params;
