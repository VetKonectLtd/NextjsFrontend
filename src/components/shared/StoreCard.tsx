  ...
  return (
    <div className="bg-white rounded-2xl shadow-md flex flex-col relative">
        ...
        <div>
          <Link href={`/dashboard/stores/${id}`} className="">
            <span className="absolute top-3 left-3 z-20 px-5 py-1 bg-white rounded-md shadow-md">
              <Eye
                size={14}
                className=" bg-white shadow-md cursor-pointer z-10"
              />
            </span>
          </Link>
        </div>
      ...
      <div
        ...
        className="rounded-t-xl cursor-pointer overflow-hidden w-full h-[190px] mb-1 relative"
      >
        <Image
          src={picture_url}
          alt={store_name}
            ...
            <div className="flex flex-col">
              <div className="flex">
                <span className="text-sm font-semibold text-gray-900 truncate max-w-[120px]">
                  {store_name.length > 18
                    ? `${store_name.slice(0, 18)}...`
                    : store_name}
                </span>
              </div>
