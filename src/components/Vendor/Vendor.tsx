...
const GENERIC_VENDOR_IMAGE = "https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=400&h=400&fit=crop";
...
const Vendor: React.FC<VendorProps> = () => {
	...
	const [selectedVendor, setSelectedVendor] = useState<VetProfileProps | null>(null);
	...
	const { data: apiData, isLoading, error, isFetching } = useGetAllVendor(currentPage, true);
	...
	const transformedVendors: VetProfileProps[] = useMemo(() => {
		...
		return combinedVendors.map((vendor: VendorData) => {
			...
			const location = `${vendor.user.state || ''}, ${vendor.user.country || ''}`.replace(/^,\s*|,\s*$/g, '');
