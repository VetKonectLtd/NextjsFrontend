...
const PetCard = ({
	...
	const { useDeletePetOwner } = usePetOwnerService();
	const deletePetMutation = useDeletePetOwner(id as string);
	const [showDetails, setShowDetails] = useState(false);
	const router = useRouter();

	const handleDeletePet = () => {
		if (window.confirm(`Are you sure you want to delete ${pet_name}?`)) {
			deletePetMutation.mutate();
		}
	};

	const handleEdit = (id: string) => {
		router.push(`/dashboard/pet/${id}/edit`);
	};
	return (
			...
			<div className="flex md:flex-row flex-col-reverse justify-center items-center md:justify-between">
				...
				<div className="flex md:flex-row items-center flex-col gap-4">
					...
					<div className="md:text-left text-center">
						<h2 className="font-bold text-lg">{pet_name}</h2>
						<p className="text-xs text-gray-400">{pet_id}</p>
					</div>
				</div>
				...
				{showDetails && (
					...
					>
						<p className="flex md:flex-row justify-center items-center md:justify-between flex-col">
							<span className="font-semibold">Species</span> {specie}
						</p>
