...
const PostDetail = ({ postId, slug }: PostDetailProps) => {
  ...
  const redirectToLogin = (action: string) => {
    ...
    router.push(
      `/login?redirect=${encodeURIComponent(currentUrl)}&action=${action}`,
    );
  };
  ...
  return (
    <div className="">
      <div>
        <ArrowLeft
          className="h-9 w-10 cursor-pointer border border-gray-200 rounded-full p-2 mb-2"
          onClick={handleBack}
        />
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-3 mb-6 shadow-sm">
        ...
        <div className="flex flex-col relative items-center gap-3 mt-4">
          <textarea
            ref={commentInputRef}
            className={`border relative outline-none shadow-md w-full p-4 text-sm font-normal py-3 rounded-md resize-none ${
              !isAuthenticated
            ...
            name="comment"
            placeholder={
              !isAuthenticated ? "Login to comment..." : "Write a comment..."
            }
            id="comment"
            rows={7}
            value={commentText}
          ...
          <button
            ...
            className={`w-full text-center text-sm text-white px-4 py-2 rounded-lg flex justify-center items-center gap-2 ${
              !isAuthenticated
                ? "bg-gray-400 cursor-not-allowed"
                : "bg-primary-400"
            }`}
            disabled={
              !isAuthenticated || (!commentText.trim() && isAuthenticated)
            }
          >
