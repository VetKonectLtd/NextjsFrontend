...
const ParaprofessionalSwitchModal = ({
  open,
  onClose,
  onSuccess,
}: ParaprofessionalSwitchModalProps) => {
  const { useSwitchToParaprofessional } = useRoleSwitchingService();
  const { useCurrentUser } = useAuthService();
  const { coordinates, error: geoError } = useGeolocation();
  const [locLoading, setLocLoading] = useState(false);

  const switchMutation = useSwitchToParaprofessional();
      ...
      <DialogContent className="max-w-sm rounded-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="text-center px-7 m-auto">
            <DialogTitle className="font-extrabold text-2xl">
              Switch to Paraprofessional
            </DialogTitle>
            <DialogDescription>
              Please provide the required information to create your
              paraprofessional profile.
            </DialogDescription>
          </div>
        </DialogHeader>
        ...
        <form className="space-y-1">
          ...
          <FormInput
            ...
            type="text"
            focusLabel="Name of Institution (Required) :"
            isRequired
          ...
          <FormInput
            ...
            type="text"
            focusLabel="Graduation Year (Required) :"
            isRequired
          ...
          <FormInput
            ...
            type="text"
            focusLabel="Expected Year of Graduation (Required) :"
            isRequired
            error={errors.expected_year_of_graduation?.message}
            {...register("expected_year_of_graduation", {
              required: "Expected year of graduation is required",
            })}
          />
...
export default ParaprofessionalSwitchModal;
