...
const VeterinarianSwitchModal = ({
	open,
	onClose,
	onSuccess,
}: VeterinarianSwitchModalProps) => {
	const { useSwitchToVeterinarian } = useRoleSwitchingService();
	const { useCurrentUser } = useAuthService();
	const { coordinates, error: geoError } = useGeolocation();
	const [locLoading, setLocLoading] = useState(false);

	const switchMutation = useSwitchToVeterinarian();
	...
	const handleUseMyLocation = async () => {
		try {
			...
			// ignore; user can provide manual coordinates
		} finally {
			setLocLoading(false);
		}
	};
					...
					<div className="flex flex-col mt-4 gap-3">
						<Button
							...
							disabled={
								...
								!(
									typeof (user as any) !== "undefined" &&
									(typeof coordinates?.latitude !== "undefined" || true)
								)
							}
...
export default VeterinarianSwitchModal;
