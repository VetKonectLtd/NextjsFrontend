...
const ClinicAccount = ({
  ...
  const currentUserId =
    (currentUser as any)?.profile.user_id ?? (currentUser as any)?.data?.id;
  const isOwnProfile =
    currentUserId &&
    selectedClinic?.id &&
    String(currentUserId) === selectedClinic?.userId &&
    String(selectedClinic?.id);
  ...
  return (
    <div className="mt-12 pb-3 text-center w-full m-auto text-gray-500 text-sm">
      ...
      {selectedClinic?.role !== "Clinic" && selectedAction === "message" && (
      ...
      {selectedAction === "mail" && (
        <>
          ...
          <p className="text-sm mt-2">{selectedClinic?.email}</p>
          <div className="flex items-center py-3 justify-center flex-col">
            <button
              onClick={() => handleCopy(selectedClinic?.email || "")}
              className="p-2 rounded-full border hover:bg-gray-100 transition"
              title="Copy to clipboard"
            >
              <Copy className="w-7 h-7" />
      ...
      {selectedAction === "location" && (
        <>
          ...
          <div className="flex items-center py-3 justify-center flex-col">
            ...
            <span className="text-xs text-gray-55">
              {copied === selectedClinic?.address ? "Copied!" : "Click to copy"}
            </span>
          </div>
        </>
      )}
      ...
      {selectedAction === "share" && (
        <>
          ...
          <p className="text-gray-800">
            https://www.vetkonect.com/dashboard/vet-vendor?clinic=
            {selectedClinic?.id}&category=Vet+Clinic
          </p>
          <div className="flex items-center py-3 justify-center flex-col">
            <button
              onClick={() =>
